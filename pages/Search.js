    ...
    <div className="">
      ...
      <div className="bg-black space-x-16 flex lg:px-32 px-5 text-white font-semibold pt-3">
        <h1
          ...
          className={`cursor-pointer pb-2 px-2 ${
            router.query.category !== "projects" && "border-b-2 border-white"
          }`}
        >
          Freelancers
        </h1>
      ...
      <div
        className={`mx-5 lg:mx-16 mb-10 ${
          router.query.category === "projects" && "hidden"
        }`}
      >
        <FreelancersSearch
          freelancers={data.users}
          totalpages={data.totalUserPages}
      ...
      >
        <ProjectsSearch
          ...
          totalpages={data.totalProjectPages}
          currentpage={data.page}
        />
      </div>
      <Footer />
...
export async function getServerSideProps(context) {
  ...
  const res = await fetch("http://localhost:5000/api/v1/search?page=1&size=9", {
    method: "POST",
    headers: {
