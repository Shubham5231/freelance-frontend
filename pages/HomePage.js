  ...
  return (
    <div>
      ...
      {session &&
        feed !== "nosession" &&
        feed !== "newuser" &&
        session?.user?.elanceprofile?.user && <Header page={"home"} />}

      {session &&
        feed !== "nosession" &&
        feed !== "newuser" &&
        session?.user?.elanceprofile?.user && (
          <div className={`mx-5 lg:mx-32 mb-10`}>
            ...
            <div className="flex justify-between pt-5 lg:grid lg:grid-cols-4 items-center">
              <div className="flex items-center space-x-3 lg:col-span-3">
                <h1 className="text-lg font-bold text-[#4c4c4c]">{`Welcome, ${
                  session?.user?.elanceprofile?.user &&
                  session?.user?.elanceprofile?.user[0]?.userName
                }`}</h1>
                <img
                  className="animate-wave w-10 h-10 hidden lg:flex"
                  src="https://threejs-journey.xyz/assets/images/hand-emoji-100x100.png"
                ></img>
              </div>
                  ...
                  <div className="space-y-10">
                    <div className=" items-center place-items-center flex justify-center">
                      <h1
                        ...
                        h1
                        className={`bg-yellow-400 px-5 py-2 text-white font-bold rounded-md cursor-pointer text-center hover:bg-yellow-500  ${
                          session.user.elanceprofile.user[0].userType ===
                          "client"
                            ? "hidden lg:flex"
                            : "hidden"
      ...
      {session &&
        session.user &&
        session.user.elanceprofile &&
        session.user.elanceprofile.user && <Footer className="" />}
      {(!session ||
        !session.user ||
        !session.user.elanceprofile ||
        !session.user.elanceprofile.user) && (
  ...
  if (elanceprofile && elanceprofile.user) {
    if (elanceprofile.user[0].userType === "client") {
      const freelancers = await fetch(
        "http://localhost:5000/api/v1/users/getAllUsers?page=1&size=10",
        {
          method: "POST",
          headers: {
      ...
      const projects = await fetch(
        "http://localhost:5000/api/v1/projects/getAllProjects?page=1&size=10",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
