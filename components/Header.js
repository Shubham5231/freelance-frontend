...
function Header({ page }) {
  ...
  const myProfile =
    session?.user.elanceprofile.user && session?.user.elanceprofile.user[0];
  const myNotifications = myProfile?.notifications.filter(
    (notification) => notification.notificationType !== "rejected"
  );
  ...
  const notificationDropId = isNotificationDropOpen
  ...
  async function hireRequestAction(
    action,
    notificationId,
    freelancerId,
    hireRequestId
    ...
    const url =
      action === "accept"
        ? "http://localhost:5000/api/v1/hire/agreeHireRequest"
        : "http://localhost:5000/api/v1/hire/rejectHireRequest";
  ...
  console.log(myNotifications, "this is mynotifications");

  async function handleHireReqAgreeNotification(
    notificationId,
    myId,
    triggeredById
  ) {
    setLoading(true);
    const res = await fetch("http://localhost:5000/api/v1/users/setContacted", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  async function readNotification(freelancerId, notificationId) {
    const resReadNotification = await fetch(
      "http://localhost:5000/api/v1/users/readNotification",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...
          <div className="flex items-center space-x-3">
            {!(session && session.user) && (
              <h1
                ...
                className={`cursor-pointer hover:text-[#29b2fe] text-lg font-semibold`}
              >
                Log In
              </h1>
            )}
            ...
            {session && session.user && (
              <img
                ...
                src={`${
                  page === "home"
                    ? "https://img.icons8.com/glyph-neue/128/29b2fe/home.png"
                    : "https://img.icons8.com/glyph-neue/128/000000/home.png"
                }`}
              />
            )}
            {session && session.user && (
              <img
                onClick={handleNMessageDropClick}
                className={`h-7 w-7 cursor-pointer`}
                src={`${
                  page === "messenger"
                    ? "https://img.icons8.com/material-rounded/128/29b2fe/facebook-messenger--v1.png"
                    : "https://img.icons8.com/material-rounded/128/000000/facebook-messenger--v1.png"
                }`}
              />
            )}
            {session && session.user && (
              ...
              >
                <div className="w-52">
                  <h1 className="text-center p-2 border-b border-[#c4c4c4] text-base font-semibold">
                    {myContactedUsers && myContactedUsers.length > 0
                      ? "Connected users"
                      : "You are not connected to anyone. Go to someones profile and click on 'Message' to add them here"}
                  </h1>
                  {myContactedUsers &&
                      ...
                      <div className="p-2 border-b border-[#c4c4c4] cursor-pointer space-y-2">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <h1
                            onClick={() =>
                              router.push(
                                `/Messenger?userId=${contactedUser._id}`
                              )
                            }
                            className="text-white p-1 bg-[#29b2fe] text-xs rounded-full font-bold hover:bg-white hover:text-[#29b2fe] hover:underline "
                          >
                            Open Messenger
                          </h1>
                        </div>
            ...
            {session && session.user && (
              <div className="flex" onClick={handleNotificationDropClick}>
                <img
                  className="h-7 w-7 cursor-pointer"
                  src="https://img.icons8.com/ios-filled/151280/000000/appointment-reminders--v1.png"
                />
                        ...
                        <div className="p-2 border-b border-[#c4c4c4] hover:bg-[#ECF0F1] cursor-pointer">
                          <h1>
                            ...
                            {(notification.notificationType ===
                              ...
                              <div
                                onClick={() =>
                                  handleJobApplicationNotification(
                                    notification._id,
                                    myProfile._id,
                              ...
                              >
                                <h1 className="text-sm font-semibold">
                                  {notification.triggeredBy.fullName} has
                            ...
                            {notification.notificationType ===
                              ...
                              <div>
                                <h1
                                  onClick={() =>
                                    router.push(
                                      `/ProjectDetails?projectId=${notification.projectId}`
                                    )
                                  }
                                  className="text-sm text-[#29b2fe] font-semibold hover:underline cursor-pointer"
                                >
                                  ...
                                  <h1
                                    onClick={() =>
                                      hireRequestAction(
                                        ...
                                        notification._id,
                                        myProfile._id,
                                        notification.hireRequestId
                                      )
                                    }
                                    className="text-white font-semibold text-sm px-2 py-1 rounded-md bg-[#2ECC71] cursor-pointer hover:bg-[#138643]"
                                  >
                                    Accept
                                  </h1>
                                  <h1
                                    ...
                                    className="text-white font-semibold text-sm px-2 py-1 rounded-md bg-[#E74C3C] cursor-pointer hover:bg-[#cc3c2c]"
                                  >
                                    Reject
                                  </h1>
                                </div>
                            ...
                            {notification.notificationType === "review" && (
                              ...
                              >
                                <h1 className="text-sm font-semibold">
                                  You have received a review from{" "}
                                  {notification.triggeredBy.fullName}
                                </h1>
                              </div>
                            )}
                            {notification.notificationType ===
                              ...
                              <div>
                                ...
                                <h1
                                  onClick={() =>
                                    handleHireReqAgreeNotification(
                                      notification._id,
                                      myProfile._id,
                                      notification.triggeredBy._id
                                    )
                                  }
                                  className="text-sm font-semibold text-[#29b2fe]"
                                >
                                  Send a Message
                                </h1>
                              </div>
                            )}
                            {notification.notificationType === "hired" && (
                              <div>
                                <h1 className="text-sm font-semibold">
                                  {notification.triggeredBy.fullName} has
                  ...
                  <img
                    ...
                    src={
                      session.user.image.includes("http")
                        ? session.user.image
                        : "https://img.icons8.com/officel/128/000000/user.png"
                    }
                    loading="lazy"
                  />
                </IconDropDown>
