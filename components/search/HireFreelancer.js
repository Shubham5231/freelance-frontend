...
function HireFreelancer({
  ...
  console.log(clientprofile, "client profile");
  console.log(freelancerprofile, "freelancer profile");
  ...
  async function validateAndHire() {
    if (
      ...
      rateInpRef.current.value
    ) {
      const clientID = session.user.elanceprofile.user[0]._id;
      setIsMissingDetails(false);
      setLoading(true);
      const res = await fetch("http://localhost:5000/api/v1/hire/hireRequest", {
        method: "POST",
        headers: {
    ...
    <div>
      {!isRequestSuccess && (
        <div className={``}>
          <h1 className="border-b-2 border-[#c4c4c4] p-5 font-bold">
            You are hiring {freelancerprofile.fullName}( @
            {freelancerprofile.userName} )!
          </h1>
          <div className="px-5 py-2 space-y-2">
            <h1>What is the job you are hiring for? *</h1>
                ...
                availableProjects.map((project) => (
                  <div className="flex space-x-3 items-start">
                    <img
                      ...
                      onClick={() => setSelecedProject(project._id)}
                      src={`${
                        selectedProject === project._id
                          ? "https://img.icons8.com/ios-glyphs/48/666666/checked-checkbox.png"
                          : "https://img.icons8.com/ios-filled/50/666666/unchecked-checkbox.png"
                      }`}
                    />
          ...
          <div className="px-5 py-2 space-y-2">
            <h1>How much hourly rate are you offering? *</h1>
          ...
          <div className="px-5 py-2 space-y-2">
            <h1>For how long you want to hire? *</h1>
            <div className={`border border-[#c4c4c4] rounded-md p-2`}>
              <input
                ref={durationInpRef}
                type="number"
          ...
          <div className="px-5 py-2 space-y-2">
            <h1>Add a message to the freelancer *</h1>
            <div className={`border border-[#c4c4c4] rounded-md p-2`}>
              <input
                ref={messageInpRef}
                className="w-full focus:outline-none"
          ...
          {isMissingDetails && (
            <h1 className=" text-center text-base font-semibold text-[#E74C3C]">
          ...
          <div className="grid grid-cols-3 p-3">
            <div />
            <h1
              onClick={validateAndHire}
              className="lg:px-4 lg:py-2 p-2 border border-[#29b2fe] bg-[#29b2fe] rounded-full text-center justify-center cursor-pointer text-white font-semibold hover:text-[#29b2fe] hover:bg-white"
            >
              Send Hire Request
            </h1>
            <div />
      ...
      {isRequestSuccess && (
        <div className="p-5 space-y-3 ">
          <h1 className="text-center text-2xl font-semibold text-[#29b2fe]">
            You hire request has been sent !
          </h1>
