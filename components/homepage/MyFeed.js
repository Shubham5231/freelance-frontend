...
function MyFeed({ feed }) {
  ...
  const myAppliedProjects = session.user.elanceprofile.user[0].applications.map(
    (application) => application.projectId._id
  );
  ...
  async function nextPage() {
    if (currPage < feed.totalPages) {
      ...
      const url =
        userType === "client"
          ? `http://localhost:5000/api/v1/users/getAllUsers?page=${
              parseInt(currPage) + 1
            }&size=10`
          : `http://localhost:5000/api/v1/projects/getAllProjects?page=${
              parseInt(currPage) + 1
            }&size=10`;
          ...
          <div className="justify-between w-full">
            ...
            <div className="flex items-center space-x-5 mt-10 justify-center">
              <img
                ...
                src={`${
                  currPage === "1"
                    ? "https://img.icons8.com/ios-glyphs/120/999999/previous.png"
                    : "https://img.icons8.com/ios-glyphs/120/29b2fe/previous.png"
                }`}
              />
              <h1>
              ...
              <img
                onClick={nextPage}
                className={`w-8 h-8 ${
                  parseInt(currPage) === parseInt(Math.ceil(totalPages))
                    ? "cursor-not-allowed"
                    : "cursor-pointer"
                }`}
                src={`${
                  currPage < totalPages
                    ? "https://img.icons8.com/ios-glyphs/120/29b2fe/next.png"
                    : "https://img.icons8.com/ios-glyphs/120/999999/next.png"
                }`}
              />
            </div>
          </div>
