...
function CustomSlider() {
  ...
  const sliderItems =
    userType === "client"
      ? session?.user.elanceprofile.user[0].projects
      : session?.user.elanceprofile.user[0].applications;
  ...
  async function sendReminder(applicationId) {
    ...
    const res = await fetch(
      "http://localhost:5000/api/v1/hire/remindJobApplication",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  console.log(sliderItems, "slideritsm");
  return (
    <div className="">
      <div className="space-y-10">
        <h1 className="text-xl text-black font-bold">
          {userType === "client" ? "My Postings" : "My Applications"}
        </h1>
            ...
            >
              {sliderItems.length === 0 && userType === "client" && (
                <div
                  ...
                  className="space-y-5 border border-[#c4c4c4] rounded-md cursor-pointer w-20"
                >
                  <h1 className="text-center p-2 border-b border-[#c4c4c4]">
                    Nothing to Show - Post a Project
                  </h1>
                  <div className="flex justify-center">
                    <img
                      className="h-28 w-28 my-10"
                      src="https://img.icons8.com/material-outlined/500/000000/add.png"
                    />
                  </div>
                </div>
              )}
              {sliderItems.length === 0 && userType === "freelancer" && (
                ...
                >
                  <h1 className="text-center p-2 border-b border-[#c4c4c4]">
                    No Applications - Search and Apply
                  </h1>
                  <div className="flex justify-center">
                    <img
                      className="h-28 w-28 my-10"
              ...
              {userType === "client" &&
                sliderItems &&
                sliderItems
                  .map((project) => <JobPost project={project} />)
                  .reverse()}
              {userType === "freelancer" &&
                sliderItems &&
                sliderItems
                    ...
                    <div className="px-3 h-96">
                      <div className="rounded-md cursor-pointer h-96 border border-[#e4e4e4] hover:border-[#c4c4c4]">
                        <div className="space-y-2 h-80">
                          <div className="space-y-2 px-3 py-1">
                            <h1
                              onClick={() =>
                                router.push(
                                  `/ProjectDetails?projectId=${application.projectId._id}`
                                )
                              }
                              className="text-[#29b2fe] text-lg font-semibold line-clamp-1 hover:underline"
                            >
                          ...
                          <div className="text-sm px-3 pb-3 pt-1">
                            <h1>
                              {application.projectId.budget?.minPrice} per Hr
                              for a freelancer |{" "}
                              {application.projectId.budget?.maxPrice} total
                              project budget
                            </h1>
                          </div>
                          <div className="px-3 py-1">
                            ...
                            <h1 className="text-base italic">
                              ...
                              {application.applicationId.bid * 9} for complete
                              project
                            </h1>
                          </div>
                        </div>
                        {!(
                          ...
                          <div
                            onClick={() =>
                              sendReminder(application.applicationId._id)
                            }
                            className="flex justify-end pb-3 px-3"
                          >
                            <h1
                              className={`bg-[#29b2fe] px-2 py-1 text-white font-semibold hover:bg-[#238ac2] rounded-full cursor-pointer text-center`}
                            >
                              {"Remind"}
                            </h1>
                          </div>
                        ...
                        {application.applicationId.applicationStatus ===
                          ...
                          <div className="pb-3 px-3">
                            ...
                            <h1
                              onClick={() =>
                                handleMessage(application.projectId.postedBy)
                              }
                              className={`text-[#29b2fe] font-semibold hover:underline`}
                            >
